...
public class IOSSLoggedInPaymentScript {
    ...
    public String executeSeleniumScript(boolean demo, boolean takeScreenShot, String govGatewayID, int percentPayment) throws IOException, InterruptedException {
        ...
        if (!paymentReferenceWebEle.isEmpty()) {
            ...
            String accountDetailsCreated = govGatewayID + '\t' + IOSSNumber + '\t' + paymentReference + '\t' + amountToPayString + '\t' + '\t' + createdAt + '\t' + "false" + '\t' + '\t' + "none";
            ...
            if (takeScreenShot){
                ...
                // Save screenshot
                try {
                    FileHandler.copy(screenshotFile, new File("evidence/screenshots/IOSS/Payments/IOSSPayment_"+paymentReference +".png"));
                    result += "Screenshot Saved to: " + "evidence/screenshots/IOSS/Payments/IOSSPayment_"+paymentReference +".png" +'\n';
                } catch (IOException e) {
